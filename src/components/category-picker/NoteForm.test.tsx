...
describe('NoteForm', () => {
  it('submits the typed note', async () => {
    const onStart = vi.fn()
    render(<NoteForm categoryId="work" onBack={() => {}} onStart={onStart} />)
  ...
  it('starts with an empty note when Enter is pressed immediately', async () => {
    const onStart = vi.fn()
    render(<NoteForm categoryId="study" onBack={() => {}} onStart={onStart} />)
  ...
  it('calls onBack when “重选” is clicked', async () => {
    const onBack = vi.fn()
    render(<NoteForm categoryId="rest" onBack={onBack} onStart={() => {}} />)
